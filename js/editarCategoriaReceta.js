...
$('#fecha').val(time);
	...
	var idCategoriaReceta = window.location.search;
	idCategoriaReceta = idCategoriaReceta.replace("?","");

	if (idCategoriaReceta) {
		traerID(idCategoriaReceta);
	}else{
		var categoriaRecetaLogueada = sessionStorage.getItem("categoriaRecetaLogueada");
		if (categoriaRecetaLogueada) {
			categoriaRecetaLogueada = JSON.parse(categoriaRecetaLogueada);
			traerID(categoriaRecetaLogueada.idcategoria_recetas);
		}
	}
