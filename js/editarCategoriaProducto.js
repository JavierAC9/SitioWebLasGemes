...
$('#fecha').val(time);
	...
	var idCategoriaProducto = window.location.search;
	idCategoriaProducto = idCategoriaProducto.replace("?","");

	if (idCategoriaProducto) {
		traerID(idCategoriaProducto);
	}else{
		var categoriaProductoLogueado = sessionStorage.getItem("categoriaProductoLogueado");
		if (categoriaProductoLogueado) {
			categoriaProductoLogueado = JSON.parse(categoriaProductoLogueado);
			traerID(categoriaProductoLogueado.idcategoria_productos);
		}
	}
